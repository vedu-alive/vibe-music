...
const FeaturesSection = () => {
  const { featuredSongs, featuredError, isFeaturedLoading } = useMusicStore();
  ...
  if (featuredError) {
    return <p className="text-red-500 mb-4 text-lg">{featuredError}</p>;
  }
      ...
      {featuredSongs.map((song) => (
        ...
        >
          <img
            src={song.imageUrl}
            alt={song.title}
            className="w-16 sm:w-20 h-20 sm:h-20 object-cover flex-shrink-0"
          />
